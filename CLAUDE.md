...
- **Frontend** is Astro with no client framework (React was removed): components are `.astro`, rendered on the server; the few interactive bits are small `<script>`s (live search on `/books`, mobile menu). Pagination is plain `?page=N` links. Component tests render with Astro's container API (`src/test/render.ts`); `bun run typecheck` is `astro check`.
- **Cover thumbnails**: saving a `Book.cover_image` writes 160/240/400px WebP copies to `media/thumbs/` (`stations/covers.py`, via a post_save signal); the API exposes them as `cover_thumbnails` for `srcset`. The deploy runs `generate_cover_thumbnails` to backfill (idempotent).
- **Page cache**: nginx caches rendered pages for 60s (`proxy_cache` in `nginx/nginx.conf.react`), serving stale while refreshing; responses carry `X-Cache: HIT/MISS/STALE/BYPASS`. The deploy clears it. Send `X-Cache-Bypass: 1` to reach the app directly (the watchdog, deploy smoke test and uptime check do). Changing the cache path needs an nginx restart, not a reload.
